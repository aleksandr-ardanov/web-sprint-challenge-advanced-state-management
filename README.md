...
It helps us avoid props drilling in big projects with large amount of data and lots of components nested.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

actions - objects which contain information for dispatch function for reducer( what to do)
reducer - it's a function which does something(mostly changing the state) when receives an action
store - state container which stores the whole state of the app in immutable object tree.
single source of truth - all data stored in one place

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

Redux-thunk allows us call action creators whose return functions instead of actions when needed, for example, we can work with promises.

4. What is your favorite state management system you've learned and this sprint? Please explain why!
   redux is very effective in my opinion, because we don't need to worry about side effects because we work with immutable tree
